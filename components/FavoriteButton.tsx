...
export function FavoriteButton({ roomId, initialFavorited = false }: { roomId: string; initialFavorited?: boolean }) {
  ...
  const toggle = async () => {
    if (!session) {
      ...
      const local = JSON.parse(localStorage.getItem('favorites') || '[]');
      if (isFavorited) {
        localStorage.setItem('favorites', JSON.stringify(local.filter((id: string) => id !== roomId)));
      } else {
